...
from rau.models.stack_nn.differentiable_stacks.nondeterministic import TooManyUpdates
from rau.models.stack_nn.differentiable_stacks.vector_nondeterministic import (
    VectorNondeterministicStack,
)
from rau.models.stack_nn.rnn.vector_nondeterministic import VectorNondeterministicStackRNN
...
def test_single_run():
    ...
    S = 4
    m = 2
    stack = VectorNondeterministicStack.new_empty(
        batch_size=B,
        num_states=Q,
...
def test_spurious_run_has_zero_weight():
    device = torch.device('cpu')
    semiring = log
    B = 1
    Q = 3
...
def test_unlimited_length():
    ...
    generator = torch.manual_seed(123)
    bottom_vector = torch.full((m,), -math.inf)
    unlimited_stack = VectorNondeterministicStack.new_empty(
        batch_size=B,
        num_states=Q,
        stack_alphabet_size=S,
        stack_embedding_size=m,
    ...
    limited_stack = VectorNondeterministicStack.new_empty(
        batch_size=B,
        num_states=Q,
        stack_alphabet_size=S,
        stack_embedding_size=m,
