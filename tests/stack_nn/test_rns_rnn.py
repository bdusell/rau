...
from rau.models.stack_nn.rnn.nondeterministic import NondeterministicStackRNN
...
from rau.models.stack_nn.differentiable_stacks.semiring import real, log
...
def test_against_reference():
    ...
    for p, reference_p in zip(model.parameters(), reference_model.parameters(), strict=True):
...
def test_return_actions_and_readings():
    batch_size = 5
    input_size = 7
    num_states = 2
    stack_alphabet_size = 3
...
def test_validate_sequence_length():
    ...
    semiring = log
    generator = torch.manual_seed(123)
    stack = NondeterministicStack.new_empty(
        batch_size=B,
        num_states=Q,
...
def test_log_matches_real():
    ...
    generator = torch.manual_seed(123)
    real_stack = NondeterministicStack.new_empty(
    ...
    log_stack = NondeterministicStack.new_empty(
        batch_size=batch_size,
        num_states=num_states,
        stack_alphabet_size=stack_alphabet_size,
        sequence_length=sequence_length,
...
def test_viterbi():
    ...
    for i in range(1, sequence_length):
        ...
        for path in paths:
            assert len(list(path)) == i
            for operation in path:
                assert isinstance(operation, Operation)
        assert scores.size() == (batch_size,)

def test_hardcoded_best_runs():
...
class ReferenceNondeterministicStackRNN(NondeterministicStackRNN):
    ...
    def initial_stack(self, batch_size, sequence_length):
